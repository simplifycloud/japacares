...
const STATS = [
  { icon: Heart, label: "Happy Mothers", value: "2,000+" },
  { icon: Star, label: "Avg. Rating", value: "4.9/5" },
  { icon: ShieldCheck, label: "Verified Caregivers", value: "100%" },
];
...
export default function Hero({ onOpenBooking }: { onOpenBooking?: () => void }) {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-[#FFF8F3] via-[#FFF3EC] to-[#FDE8E4] min-h-screen flex items-center pt-20 md:pt-24">
      {/* Background Blobs */}
      <div
      ...
      <div className="relative max-w-7xl mx-auto px-6 md:px-12 py-12 md:py-16 grid md:grid-cols-2 gap-8 md:gap-16 items-center w-full">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 md:px-4 py-2 rounded-full bg-white/70 backdrop-blur-md border border-rose-100 shadow-sm mb-4 md:mb-6">
            ...
            <span className="text-[10px] md:text-xs font-semibold text-gray-700 tracking-wide uppercase">
          ...
          <div className="flex flex-wrap gap-6 md:gap-8 mt-10 md:mt-14 pt-6 md:pt-8 border-t border-gray-200/70">
            {STATS.map(({ icon: Icon, label, value }) => (
              <div key={label} className="flex items-center gap-2 md:gap-3">
                <div
                  className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-white shadow-sm border border-gray-100 flex items-center justify-center"
                  aria-hidden="true"
                >
                  <Icon size={16} className="text-rose-500" />
                </div>
                <div>
                  <p className="font-serif text-lg md:text-xl font-bold text-gray-900 leading-none">
                    {value}
                  </p>
        ...
        >
          ...
          >
            <div
              className="w-8 md:w-11 h-8 md:h-11 rounded-full bg-gradient-to-br from-rose-400 to-pink-500 flex items-center justify-center text-white shadow-md flex-shrink-0"
              aria-hidden="true"
            >
